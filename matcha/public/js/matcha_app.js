...
function MatchaApp() {
			...
			if (bankAccount && (toDate || fromDate) && company) {
				...
				try {
					...
					const balance = await fetchJSON(
						"matcha.api.transactions.get_account_balance",
						{
							bank_account: bankAccount,
							till_date: till,
							company: company,
		...
		() =>
			transactions.filter((tx) => {
				...
				if (statusFilter !== "All") {
					...
					if (statusFilter === "Unreconciled" && status !== "Not Reconciled")
						return false;
					if (
						statusFilter === "Partially Reconciled" &&
				...
				h(
					...
					h(
						...
						[
							...
							"Status",
							"Actions",
						].map((hLabel) => h("th", { key: hLabel }, hLabel))
					)
				),
				h(
					"tbody",
	...
	function renderSummaryCards() {
		const bankVal =
			closingBalanceAsPerBank === "" ? null : parseFloat(closingBalanceAsPerBank);
		const cleared = clearedBalance != null ? clearedBalance : null;
		const diff = bankVal != null && cleared != null ? bankVal - cleared : null;
		...
		return h(
			...
			h(
				...
				h(
					"div",
					{ className: "matcha-details-title", style: { fontSize: "0.8rem" } },
	...
	function renderListRegion() {
		return h(
			...
			error &&
				h(
					"div",
					{ className: "text-red-600 text-sm", style: { marginTop: "0.5rem" } },
					error
				),
			renderSummaryCards(),
			renderTable(),
