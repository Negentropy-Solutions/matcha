...
def get_payments(
	...
	if search:
		s = f"%{search.strip()}%"
		q = q.where((pe.party.like(s)) | (pe.name.like(s)) | (pe.reference_no.like(s)))
	if from_date:
		...
		try:
			...
			if party_type in ("Employee", "Shareholder") and party:
				try:
					...
					if party_account:
						accounts_list = (
							[party_account] if isinstance(party_account, str) else (party_account or [])
						)
						if accounts_list:
	...
	for r in rows:
		...
		out.append(
			_payment_row(
				...
				currency=currency or "",
				receivable_payable_account=receivable_payable_account,
				exchange_rate=r.source_exchange_rate
				if r.payment_type == "Receive"
				else r.target_exchange_rate,
	...
	if party_type and party:
		...
		for account in accounts or []:
			...
			jv_list = _get_jv_payments(
				company, party_type, party, account, from_date, to_date, (limit or 50) + 20
			)
			for p in jv_list:
...
def get_exceptions(
	...
	if search_payments:
		...
		pe_query = pe_query.where((pe.party.like(search_term)) | (pe.name.like(search_term)))
	pe_rows = (
		pe_query.orderby(pe.posting_date, order=Order.asc).offset(offset_payments).limit(limit_per_type)
	).run(as_dict=True)
	...
	si_or = (
		[["customer", "like", "%" + search_invoices + "%"], ["name", "like", "%" + search_invoices + "%"]]
		if search_invoices
		else None
	)
	if search_invoices:
	...
	pi_or = (
		[["supplier", "like", "%" + search_invoices + "%"], ["name", "like", "%" + search_invoices + "%"]]
		if search_invoices
		else None
	)
	if search_invoices:
...
def reconcile(
	...
	**kwargs,
	...
	for alloc in allocation_list:
		...
		entry.difference_account = frappe.get_cached_value("Company", company, "exchange_gain_loss_account")
	...
	if preview_only:
		...
		for alloc in allocation_list:
			...
			p_rate = (
				flt(
					pay_d.source_exchange_rate
					if pay_d.payment_type == "Receive"
					else pay_d.target_exchange_rate
				)
				or 1.0
			)
			p_cur = pay_d.paid_from_account_currency
	...
	for inv in pr.invoices:
		if inv.invoice_number == (
			allocation_details[0].get("invoice_number") if allocation_details else None
		):
...
def _merge_unreferenced_jea_rows(jv_name: str, cleared_row_names: list[str]) -> None:
	...
	for _sig, rows in groups.items():
		...
		if len(survivor_ples) > 1:
			...
			(qb.from_(ple_t).delete().where(ple_t.name.isin(ple_to_delete))).run()
		...
		(qb.from_(jea_t).delete().where(jea_t.name.isin(to_delete))).run()
...
def undo_payment_reconciliation(log_name: str | int) -> dict:
	...
	for (voucher_type, voucher_no), allocs in groups.items():
		invoice_refs = {
			(a.get("invoice_type"), a.get("invoice_number")) for a in allocs if a.get("invoice_number")
		}
		if not invoice_refs:
			continue
		...
		unrecon.allocations = [
			a for a in unrecon.allocations if (a.reference_doctype, a.reference_name) in invoice_refs
		]
...
def reconcile_xco(
	...
	for alloc in alloc_list:
		...
		if not xco_receivable_account:
			frappe.throw(
				_("Could not determine receivable/payable account for {0} in {1}").format(
		...
		frappe.db.set_value(
			"Journal Entry", jv_pay.name, "inter_company_journal_entry_reference", jv_inv.name
		)
		...
		pay_row = next((p for p in pr_pay.payments if p.reference_name == payment_name), None)
		jv_row = next((i for i in pr_pay.invoices if i.invoice_number == jv_pay.name), None)

		if not pay_row:
			frappe.throw(
				_("Payment Entry {0} not found as unreconciled in {1}").format(payment_name, payment_company)
			)
		if not jv_row:
			frappe.throw(
				_(
					"Journal Entry {0} not found as outstanding in {1} — "
					"please check that the intercompany bridge account is correct"
				).format(jv_pay.name, payment_company)
			)
		...
		entry["difference_amount"] = pr_pay.get_difference_amount(pay_dict, inv_dict, allocated_amount)
		entry["exchange_rate"] = inv_dict.get("exchange_rate")
		exc_gain_loss_posting_date = frappe.db.get_single_value(
			"Accounts Settings", "exchange_gain_loss_posting_date", cache=True
		)
